...
export class DiscordBot extends BaseBot {
    ...
    private async processMention(mention: DiscordMention): Promise<void> {
        ...
        try {
            // Structured logging: Start processing
            console.log(JSON.stringify({
                event: 'mention.processing.start',
                user: mention.user,
                channel: mention.channel,
            ...
            try {
                ...
                // Structured logging: API error
                console.error(JSON.stringify({
                    event: 'claude.api.error',
                    user: mention.user,
            ...
            console.log(JSON.stringify({
                event: 'mention.processing.success',
                user: mention.user,
                channel: mention.channel,
                messageId: mention.id,
                durationMs: duration,
            ...
            // Structured logging: Error with full context
            const duration = Date.now() - startTime;
            console.error(JSON.stringify({
                event: 'mention.processing.error',
                user: mention.user,
                channel: mention.channel,
                messageId: mention.id,
                error: error.message,
            ...
            // Send error message to Discord (with retry logic)
            try {
                await this.sendDiscordReply(
                    mention.channel,
                    mention.id,
                ...
                // Structured logging: Reply error
                console.error(JSON.stringify({
                    event: 'discord.reply.error',
                    user: mention.user,
                    channel: mention.channel,
    ...
    /**
     * Execute a KADI tool via broker with retry logic and structured logging
     */
    private async executeKadiTool(
        ...
        if (!this.protocol) {
            const error = {success: false, error: 'Protocol not initialized'};
            console.error(JSON.stringify({
        ...
        try {
            ...
            // Use BaseBot's invokeToolWithRetry for circuit breaker and retry logic
            ...
            // Structured logging: Tool execution error with stack trace
            const duration = Date.now() - startTime;
            console.error(JSON.stringify({
                event: 'tool.execution.error',
                toolName,
