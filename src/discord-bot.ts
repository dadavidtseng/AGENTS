...
export class DiscordBot {
  ...
  private logMetrics(): void {
    ...
    console.log('📊 Discord Bot Metrics:');
  ...
  /**
   * Start polling for Discord mentions
   */
  start(): void {
    console.log(`🤖 Starting Discord bot (polling every ${this.pollIntervalMs / 1000}s)...`);
  ...
  private async pollForMentions(): Promise<void> {
    ...
    try {
      // Get mentions from MCP_Discord_Client with retry logic
      ...
      if (mentions.length > 0) {
        console.log(`📬 Received ${mentions.length} Discord mention(s)`);
      ...
      console.error('❌ Error polling Discord mentions:', error);
    } finally {
