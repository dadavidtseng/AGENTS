...
export function AgentMonitorPage() {
  ...
  useEffect(() => {
    ...
    const handleAgentRegistered = (data: Agent) => {
      console.log('[AgentMonitor] Agent registered:', data);
      setAgents((prevAgents) => {
        // Check if agent already exists
        const exists = prevAgents.some((a) => a.agentId === data.agentId);
        if (exists) {
    ...
    const handleAgentStatusChanged = (data: {
      ...
      console.log('[AgentMonitor] Agent status changed:', data);
      setAgents((prevAgents) =>
        prevAgents.map((a) =>
          a.agentId === data.agentId
            ? { ...a, status: data.status, currentTasks: data.currentTasks }
            : a
        )
      );
    };

    apiClient.on('agent_registered', handleAgentRegistered);
    apiClient.on('agent_status_changed', handleAgentStatusChanged);

    // Cleanup: unsubscribe from WebSocket events
    return () => {
      apiClient.off('agent_registered', handleAgentRegistered);
      apiClient.off('agent_status_changed', handleAgentStatusChanged);
    };
  }, []);
