 ...
 *
 * FIXED: Always return complete output to match OutputSchema validation.
 * The MCP SDK validates responses against the declared schema, which requires
 * all fields to be present. Returning partial objects causes validation errors.
 *
 * Verbosity levels are preserved for future use but currently return full output.
 */
function filterGitStatusOutput(
  result: ToolOutput,
  level: VerbosityLevel,
): ToolOutput {
  // Always return complete output to satisfy schema validation
