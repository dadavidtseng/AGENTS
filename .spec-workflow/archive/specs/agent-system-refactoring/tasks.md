# Tasks Document: Agent System Refactoring

## Architecture Overview

### Agent Hierarchy and Relationships

**BaseBot (Existing - Will be Moved)**
- **Location**: Currently `C:\p4\Personal\SD\AGENTS\shared\base-bot.ts`, will move to `agents-library\src\base-bot.ts`
- **Purpose**: Abstract base class providing circuit breaker, retry logic, and metrics tracking
- **Key Features**: Fault tolerance (circuit breaker pattern), exponential backoff, Anthropic Claude API client, KĀDI protocol tool invocation
- **Not Extended By**: BaseBot is **NOT** a parent class for agents. It provides utilities that agents **compose with** or **delegate to**.

**BaseWorkerAgent (New - Will be Created)**
- **Location**: `agents-library\src\worker-agent-factory.ts`
- **Purpose**: Base class for task-executing worker agents (artist, designer, programmer)
- **Relationship to BaseBot**: **Composes with BaseBot** for circuit breaker and retry functionality (e.g., calls `BaseBot.retryWithBackoff()`)
- **Key Responsibilities**: KĀDI task subscription, Claude API task execution, git operations, event publishing
- **Instantiation**: Created via `WorkerAgentFactory.createAgent(config)` with role-specific configuration

**BaseShadowAgent (New - Will be Created)**
- **Location**: `agents-library\src\shadow-agent-factory.ts`
- **Purpose**: Base class for backup/monitoring shadow agents
- **Relationship to BaseBot**: **Composes with BaseBot** for circuit breaker resilience during backup operations
- **Key Responsibilities**: Filesystem watching (chokidar), git ref monitoring, shadow worktree mirroring, backup event publishing
- **Instantiation**: Created via `ShadowAgentFactory.createAgent(config)` with shadow-specific configuration

**agent-producer (Different Architecture - Not Derived)**
- **Location**: `C:\p4\Personal\SD\agent-producer`
- **Purpose**: Orchestrator that coordinates worker agents via tool registration
- **Architecture**: KĀDI tool provider (registers plan_task, assign_task, etc.), not a task executor
- **Relationship to BaseBot**: Uses BaseBot's circuit breaker utilities but is NOT an agent instance
- **Relationship to agents-library**: Will use `ProducerToolUtils` for common patterns (invokeShrimTool, orchestrateWithClaude)
- **Key Difference**: agent-producer is an orchestrator/coordinator, not a worker/shadow agent

### Why No Single BaseAgent?

The design intentionally avoids creating a single `BaseAgent` parent class because:

1. **Worker vs Shadow Have Fundamentally Different Lifecycles**:
   - Workers: Subscribe to task events → Execute with Claude → Commit to git
   - Shadows: Watch filesystem → Mirror commits → Publish backup status
   - Shared base class would force artificial commonality

2. **agent-producer Is Not An Agent Type**:
   - agent-producer is a tool provider/orchestrator
   - Does not execute tasks or mirror commits
   - Shares utility patterns (tool invocation, Claude orchestration) but not agent lifecycle

3. **Composition Over Inheritance**:
   - BaseBot already provides reusable utilities via **composition** (agents call BaseBot methods)
   - Strategy pattern allows role-specific behaviors without inheritance
   - Factories inject dependencies, avoiding tight coupling

4. **Existing BaseBot Serves as Utility Library**:
   - BaseBot's purpose: Provide circuit breaker, retry, metrics
   - BaseWorkerAgent and BaseShadowAgent **use** BaseBot as a utility, not as a parent class
   - This keeps BaseBot focused and prevents "god class" anti-pattern

### Summary

- **BaseBot**: Utility library for resilience patterns (already exists)
- **BaseWorkerAgent**: Worker agent base class created by WorkerAgentFactory (composes with BaseBot)
- **BaseShadowAgent**: Shadow agent base class created by ShadowAgentFactory (composes with BaseBot)
- **agent-producer**: Orchestrator using ProducerToolUtils (different architecture, not an agent)
- **No BaseAgent**: Intentional design - composition over inheritance, different lifecycles
