# Design Document: Simple Multi-Agent Orchestration
...
3. **SlackBot and DiscordBot (from template-agent-typescript)**
...
**Architecture:** KĀDI Agent (NOT an MCP server)
- Registers tools with KĀDI broker
- Claude Code/Desktop users invoke tools via KĀDI broker
- agent-producer forwards requests to mcp-shrimp-task-manager (via `kadiClient.load()`)
- agent-producer publishes events to worker agents

**Implementation:**
- **File**: `agent-producer/src/index.ts` (KĀDI agent pattern)
- **Dependencies**: KadiClient, existing SlackBot/DiscordBot
- **Does NOT extend**: BaseBot (already has bot integrations)
- **Pattern**: Clone from template-agent-typescript, customize with orchestration logic

**Tool Flow:**
```
Claude Code User
  ↓ calls tool via KĀDI broker
KĀDI Broker (proxies tools)
  ↓ routes to agent-producer
agent-producer (KĀDI Agent)
  ↓ kadiClient.load('mcp-shrimp-task-manager')
mcp-shrimp-task-manager (MCP upstream)
  ↓ returns task state
agent-producer
  ↓ publishes events
Worker Agents
...
**Registered Tools (via kadiClient.registerTool):**
...
**Reuses:**
- KadiClient for broker communication and tool registration (`kadiClient.registerTool()`)
- SlackBot and DiscordBot from template (customized for orchestration)
- BaseBot for circuit breaker, retry logic (via SlackBot/DiscordBot)
- `kadiClient.load('mcp-shrimp-task-manager')` to forward task management to upstream MCP server
