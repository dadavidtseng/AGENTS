...
class BackupManager {
    constructor(config) {
        ...
        this.isContainerMode = process.env.KADI_DEPLOY_MODE === 'container';
        this.arcadedbHome = process.env.ARCADEDB_HOME || '/home/arcadedb';
    }
        ...
        try {
            ...
            if (!overwrite) {
                try {
                    ...
                    if (error.message && error.message.includes('already exists')) throw error;
            ...
            if (this.isContainerMode) {
                ...
                // ArcadeDB is running as a managed Java process — we cannot
                // stop/restart it.  Instead:
                //   1. Drop the existing DB if overwrite requested
                //   2. Extract the backup ZIP into the databases/ directory
                //   3. Ask the running server to open the database
                return await this._restoreInContainerMode(databaseName, backupFilePath, overwrite);
            }
    ...
    /**
     * Restore a database while ArcadeDB is running natively (container mode).
     *
     * Since we can't restart the JVM, we:
     *   1. Drop the existing database (if overwrite)
     *   2. Extract the backup ZIP into databases/<name>/
     *   3. POST `open database <name>` to the server API so the running
     *      instance discovers the restored data — no restart needed.
     */
    async _restoreInContainerMode(databaseName, backupFilePath, overwrite) {
        const auth = Buffer.from(`${this.username}:${this.password}`).toString('base64');
        const serverUrl = `http://${this.host}:${this.port}`;

        // 1. Drop existing database if overwrite requested
        if (overwrite) {
            try {
                const exists = await this._databaseExists(databaseName);
                if (exists) {
                    console.log(`🗑️ Dropping existing database "${databaseName}" for overwrite…`);
                    const dropRes = await fetch(`${serverUrl}/api/v1/server`, {
                        method: 'POST',
                        headers: {
                            'Authorization': `Basic ${auth}`,
                            'Content-Type': 'application/json',
                        },
                        body: JSON.stringify({ command: `drop database ${databaseName}` }),
                    });
                    if (!dropRes.ok) {
                        console.log(`⚠️ Drop returned ${dropRes.status}: ${await dropRes.text()}`);
                    } else {
                        console.log(`✅ Existing database dropped`);
                    }
                    // Give ArcadeDB a moment to release file handles
                    await new Promise(resolve => setTimeout(resolve, 1000));
                }
            } catch {
                // database might not exist — safe to continue
            }
        }

        // 2. Extract the backup ZIP into the databases directory
        const databaseDir = path.join(this.arcadedbHome, 'databases', databaseName);
        fs.mkdirSync(databaseDir, { recursive: true });

        console.log(`📦 Extracting backup to ${databaseDir}…`);
        try {
            execSync(`unzip -o "${backupFilePath}" -d "${databaseDir}"`, {
                encoding: 'utf8',
                stdio: 'pipe',
            });
        ...
        // 3. Tell the running ArcadeDB server to open the database
        console.log(`📂 Opening database "${databaseName}" on running server…`);
        const openRes = await fetch(`${serverUrl}/api/v1/server`, {
            method: 'POST',
            headers: {
                'Authorization': `Basic ${auth}`,
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ command: `open database ${databaseName}` }),
        });

        if (!openRes.ok) {
            const text = await openRes.text();
            // If the server says the database already exists/is open, that's OK
            if (!text.includes('already') && !text.includes('exists')) {
                console.log(`⚠️ open database response: ${openRes.status} ${text}`);
            }
        }
    ...
    async _ensureContainerRunning() {
        ...
        if (this.isContainerMode) {
            ...
            throw new Error('ArcadeDB is not ready (container mode — server started by entrypoint)');
        }

        // Docker mode — check container
        try {
            ...
            if (error.message.includes('ArcadeDB is not ready')) throw error;
            throw new Error('Container is not running. Start it first with: node arcade-admin.js start');
        }
    }
    ...
    async _executeRestoreScript(databaseName, backupFileName) {
        ...
        let restoreCommand;
        if (this.isContainerMode) {
            // Native mode — call restore.sh directly with absolute paths
            const restoreScript = path.join(this.arcadedbHome, 'bin', 'restore.sh');
            restoreCommand = `${restoreScript} -f ${containerBackupPath} -d ${containerDatabasePath}`;
        } else {
            // Docker mode
            restoreCommand = `docker exec ${this.containerName} ` +
        ...
        try {
            const output = execSync(restoreCommand, {
                encoding: 'utf8',
                cwd: this.isContainerMode ? this.arcadedbHome : undefined,
            });
    ...
    async _stopContainer() {
        if (this.isContainerMode) {
            // In container mode, ArcadeDB is managed by the entrypoint — no-op
            console.log('ℹ️ Container mode — skipping container stop');
            return;
        }
        try {
    ...
    async _startContainer() {
        if (this.isContainerMode) {
            // In container mode, ArcadeDB is managed by the entrypoint
            // Just wait for it to be ready
            console.log('ℹ️ Container mode — waiting for ArcadeDB to be ready...');
            let attempts = 0;
            while (attempts < 60) {
                try {
                    const auth = Buffer.from(`${this.username}:${this.password}`).toString('base64');
                    ...
                    if (response.status === 204 || response.ok) {
                        console.log('✅ ArcadeDB is ready (container mode)');
                        return;
                    }
                } catch (error) { /* expected during startup */ }
                await new Promise(resolve => setTimeout(resolve, 1000));
                attempts++;
            }
            throw new Error('ArcadeDB failed to become ready (container mode)');
        }
